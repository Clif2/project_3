...
const SubmissionForm = props => {
	...
	if (!props.update) {
		...
		weather = props.weatherIcons.map((item, idx) => {
			...
			if (props.formData.weather === item.name) {
				isSelected = true;
			}
			return (
				<IconButton
						...
						<div className={css(styles.innerForm)}>
							<div className={css(styles.textArea)}>
								...
								<textarea
									...
									name="why"
								/>
								<button className={css(styles.buttonSubmit)} type="submit">
									Submit
								</button>
								{props.errorMsg && (
									<div className={css(styles.errorMsg)}>{props.errorMsg}</div>
								)}
							</div>
						</div>
					</div>
						...
						<div className={css(styles.innerForm)}>
							<div className={css(styles.textArea)}>
								...
								<textarea
									...
									name="why"
									row="20"
									placeholder={props.formData.why}
								/>
								<button className={css(styles.buttonSubmit)} type="submit">
									Update
								</button>
								<button
									className={css(styles.buttonSubmit)}
