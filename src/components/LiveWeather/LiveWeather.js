	...
	render() {
		...
		return (
			<div className={css(styles.liveWeather)}>
				<h3>
					{' '}
					<span className={css(styles.callOut)}>Current Weather</span>
					<span className={css(styles.callOutSmall)}> in</span>{' '}
				</h3>
				...
				{this.state.zipcode ? (
					<div>
						<Icon icon={theIcon} />
						<p>
							<span className={css(styles.conditionLabel)}>
								{this.state.condition}{' '}
							</span>{' '}
						</p>
						<p>
							<span className={css(styles.weatherLabel)}> Current Temp: </span>
							{this.state.temperature} &#8457;
						</p>
						<p>
							<span className={css(styles.weatherLabel)}> Feels Like: </span>
							{this.state.feelsLike} &#8457;
						</p>
						<p>
							<span className={css(styles.weatherLabel)}> Wind: </span>
							{this.state.wind} mph
						</p>
					</div>
				) : (
